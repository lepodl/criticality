...
class block:
    ...
    def __init__(self, node_property, w_uij, delta_t=1, i_mean=0.5, i_sigma=0.1, tau_i=5.):
    ...
    def update_Vi(self, delta_t):
        ...
        C_diff_Vi = main_part + self.I_syn + self.I_extern_Input + self.i_background
    ...
    def run(self, debug_mode, noise_rate=None):
        ...
        self.update_I_syn()
        self.update_i_background(self.delta_t)
    ...
    def clear(self):
        self.t_ik_last = - torch.ones([self.N], device=self.w_uij.device) * self.T_ref  # shape [N]
        self.V_i = torch.ones([self.N], device=self.w_uij.device) * (
                    self.V_th + self.V_reset) / 2  # membrane potential, shape: [N]
        self.J_ui = torch.zeros([4, self.N], device=self.w_uij.device)  # shape [K, N]
        self.t = torch.tensor(0., device=self.w_uij.device)  # scalar
