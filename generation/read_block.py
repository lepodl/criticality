...
def connect_for_block(block_dir, dense=True, bases=None, return_src=False, scaling=False):
    """
    ...
    if dense:
        ...
        # wuij = torch.cat([torch.sparse.FloatTensor(
        ...
        if scaling:
            pop = property[:, 3]
            pop_output = np.take(pop, indices[1, :])
            pop_input = np.take(pop, indices[2, :])
            # index1 = np.logical_and(pop_output==0, pop_input>=0, indices[0]==1)
            # weights[index1] = weights[index1] * 0.8
            # index1 = np.logical_and(pop_output == 0, pop_input >= 2, indices[0] == 0)
            # weights[index1] = weights[index1] * 1.3
            index1 = np.logical_and(pop_output == 2, pop_input == 2, indices[0] == 1)
            weights[index1] = weights[index1] * 1.2
            index1 = np.logical_and(pop_output == 2, pop_input == 2, indices[0] == 0)
            weights[index1] = weights[index1] * 0.8
            index1 = np.logical_and(pop_output == 2, pop_input < 2, indices[0] == 1)
            weights[index1] = weights[index1] * 1.1
            # index1 = np.logical_and(pop_output == 2, pop_input < 2, indices[0] == 0)
            # weights[index1] = weights[index1] * 0.8
        wuij = torch.sparse.FloatTensor(torch.from_numpy(indices.astype(np.int64)), torch.from_numpy(weights), torch.Size([4, bases[-1], bases[-1]]))

        assert property.shape[0] == wuij.shape[1]
        assert wuij.shape[2] == wuij.shape[1]
        print(property.shape, wuij.shape)
        if return_src:
            return property, wuij, torch.from_numpy(np.concatenate(src).astype(np.bool)) if len(src) > 0 else None
        else:
            return property, wuij
    else:
        def conn(i, s, e):
            pkl_path = os.path.join(block_dir, "block_{}.npz".format(i))
            file = np.load(pkl_path)
