...
def connect_for_single_sparse_block(population_idx, k, extern_input_rate, extern_input_k_sizes, degree=int(1e3),
    ...
    if s < e:
        if isinstance(extern_input_rate, np.ndarray):
            ...
            # assert degree_max >= degree, f"{degree_max} and {degree}"
        ...
        def _run(i):
            ...
            connect_weight = np.random.rand(degree, 2).astype(np.float32) * (init_max - init_min) + init_min
            for _idx in np.unique(input_block_idx):
                ...
                if _idx != population_idx:
                    ...
                    # if _idx % 2 == 0 and population_idx < 2:
                    #     connect_weight[extern_incomming_idx, 0] = connect_weight[
                    #                                                   extern_incomming_idx, 0] * 0.5
                    #     connect_weight[extern_incomming_idx, 1] = connect_weight[
                    #                                                   extern_incomming_idx, 1] * 1.
                    # elif _idx % 2 == 0 and population_idx >= 2:
                    #     connect_weight[extern_incomming_idx, 0] = connect_weight[
                    #                                                   extern_incomming_idx, 0] * 1.
                    #     connect_weight[extern_incomming_idx, 1] = connect_weight[
                    #                                                   extern_incomming_idx, 1] * 0.5
                else:
                    extern_outcomming_idx = get_k_idx(_extern_input_k_sizes[_idx], extern_incomming_idx.shape[0],
            ...
            return input_block_idx, input_neuron_idx, input_channel_offset, connect_weight.astype(np.float32)

        time1 = time.time()
        with Thpool() as p:
            input_block_idx, input_neuron_idx, input_channel_offset, connect_weight = tuple(zip(*p.map(_run, range(s, e))))
        time2 = time.time()
        ...
        connect_weight = np.stack(connect_weight, axis=0)
    else:
